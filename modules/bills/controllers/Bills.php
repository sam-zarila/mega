...
class Bills extends AdminController
{
    ...
    public function bills_income($year = '')
    {
        $_expenses_years = [];
        $_years          = [];
        ...
        $data['years']                           = $_years;
        $data['chart_expenses_vs_income_values'] = json_encode($this->reports_model->get_bills_vs_income_report($year));
        $data['title']                           = 'Contas a Pagar vs Renda';
        $this->load->view('reports/expenses_vs_income', $data);
    }
    ...
    public function recurring($id = '')
    {
        ...
        $data['invoiceid']            = $id;
        $data['title']                = 'Contas a Pagar Recorrentes';
        $data['invoices_years']       = $this->invoices_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->invoices_model->get_sale_agents();
        $this->load->view('recurring/list', $data);
    }
    ...
    public function table($clientid = '')
    {
        ...
        $this->app->get_table_data(($this->input->get('recurring') ? 'recurring_bills' : 'bills'), [
            'clientid' => $clientid,
            'data'     => $data,
        ]);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        try {
            if ($this->input->is_ajax_request()) {
                ...
                $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Invoices_model::STATUS_CANCELLED) ? $this->invoices_model->check_for_merge_invoice($customer_id, $current_invoice, true) : [];

                $data['merge_info'] = $this->load->view('merge_invoice', $_data, true);
                ...
                $data['expenses_bill_info'] = $this->load->view('bill_expenses', $__data, true);
    ...
    public function validate_invoice_number()
    {
        ...
        public function copy($id)
        {
            ...
            if ($new_id) {
                ...
                redirect(admin_url('bills/invoice/' . $new_id));
            } else {
                set_alert('success', _l('invoice_copy_fail'));
            }
            redirect(admin_url('bills/invoice/' . $id));
        }
            ...
            if ($this->input->post()) {
                ...
                if ($id == '') {
                    ...
                    if ($id) {
                        ...
                        $redUrl = admin_url('bills/list_invoices/' . $id);
                    ...
                    redirect(admin_url('bills/list_invoices/' . $id));
                }
            }
            if ($id == '') {
                ...
                $data['invoices_to_merge'] = $this->invoices_model->check_for_merge_invoice($invoice->clientid, $invoice->id, true);
        ...
        public function get_invoice_data_ajax($id)
        {
            ...
            $data['invoices_to_merge']          = $this->invoices_model->check_for_merge_invoice($invoice->clientid, $id, true);
            ...
            $this->load->view('invoice_preview_template', $data);
        }
        ...
        {
            if ($this->input->post()) {
                load_invoices_total_template(true);
            }
        }
        ...
        public function record_invoice_payment_ajax($id)
        {
            ...
            $data['payments'] = $this->payments_model->get_invoice_payments($id);
            $this->load->view('record_payment_template', $data);
        }
