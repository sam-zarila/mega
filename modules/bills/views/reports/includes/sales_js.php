...
$(function() {
    ...
    $('.table-proposals-report').on('draw.dt', function() {
        ...
        $('.table-invoices-report').on('draw.dt', function() {
            ...
            $('.table-bills-report').on('draw.dt', function() {
                var billReportsTable = $(this).DataTable();
                var sums = billReportsTable.ajax.json().sums;
                add_common_footer_sums($(this),sums);
                $(this).find('tfoot td.amount_open').html(sums.amount_open);
                $(this).find('tfoot td.applied_credits').html(sums.applied_credits);
                <?php foreach($invoice_taxes as $key => $tax){ ?>
                ...
                $('.table-credit-notes-report').on('draw.dt', function() {
                    ...
                    function init_report(e, type) {
                        ...
                        } else if (type == 'bills-report') {
                            report_bills.removeClass('hide');
                        }
                    ...
                    function total_bills_bar_report() {
                        if (typeof(salesChart) !== 'undefined') {
                            salesChart.destroy();
                        }
                        var data = {};
                        ...
                        $.post(admin_url + 'reports/total_bills_report', data).done(function(response) {
                            response = JSON.parse(response);
                            salesChart = new Chart($('#chart-bills'), {
                                type: 'bar',
                                data: response,
                                options: {
                                    responsive: true,
                    ...
                    function report_by_customer_groups() {
                        ...
                        function bills_report() {
                            if ($.fn.DataTable.isDataTable('.table-bills-report')) {
                                $('.table-bills-report').DataTable().destroy();
                            }
                            initDataTable('.table-bills-report', admin_url + 'reports/bills_report', false, false, fnServerParams, [
                                [2, 'desc'],
                                [0, 'desc']
                            ]).column(2).visible(false, false).columns.adjust();
                        }
                        ...
                        function gen_reports() {
                            ...
                            if (!$('.chart-income').hasClass('hide')) {
                            ...
                            } else if(!$('.chart-bills').hasClass('hide')) {
                                total_bills_bar_report();
                                ...
                                invoices_report();
                            } else if (!report_bills.hasClass('hide')) {
                                bills_report();
                            }
