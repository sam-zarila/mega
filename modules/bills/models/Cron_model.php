    ...
    private function recurring_invoices()
    {
        ...
        foreach ($invoices as $invoice) {
            ...
            if (date('Y-m-d') >= $re_create_at) {
                ...
                $new_invoice_data['bill']             = $_invoice->bill == '1' ? '1' : '0';
                ...
                if ($id) {
                    ...
                    $invoice_data = $this->invoices_model->get($id);

                    $payment_data['paymentmode'] = 'payboox';
                    $payment_data['invoice_id'] = $id;
                    $payment_data['hash'] = $invoice_data->hash;
                    $payment_data['invoice'] = $invoice_data;
                    $payment_data['auto'] = true;
                    $payment_data['amount'] = $invoice_data->total;

                    $boleto = $this->payments_model->process_payment($payment_data, $id);
