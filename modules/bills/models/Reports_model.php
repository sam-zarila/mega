...
class Reports_model extends App_Model
{
    ...
    public function get_bills_vs_income_report($year = '')
    {
        $this->load->model('expenses_model');
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $this->db->select('id')->from(db_prefix() . 'invoices')->where('MONTH(date)', $m)->where('YEAR(date)', $year)->where('bill', 1);
            ...
            if(count($bills) > 0) {
                foreach ($bills as $expense) {
                    $expense = $this->invoices_model->get($expense['id']);
                    $total   = $expense->total;
    ...
    public function total_bills_report()
    {
        $year = $this->input->post('year');
        $this->db->select('amount,' . db_prefix() . 'invoicepaymentrecords.date');
        $this->db->from(db_prefix() . 'invoicepaymentrecords');
