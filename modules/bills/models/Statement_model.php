...
class Statement_model extends App_Model
{
    ...
    public function get_statement($customer_id, $from, $to)
    {
        ...
        $invoices = $this->db->query($sql . '
            ...
            AND bill != 1 ORDER By date DESC')->result_array();
        ...
        WHERE clientid = ' . $this->db->escape_str($customer_id) . '
        AND ' . $sqlDate . ' AND status != ' . Invoices_model::STATUS_DRAFT . ' AND status != ' . Invoices_model::STATUS_CANCELLED . ' AND bill != 1')
            ->row()->invoiced_amount;
